...
private instance : Inhabited State := ⟨ arbitrary, arbitrary ⟩
...
/-- Retruns a topological ordering of any DAG (directed acyclic graph). If the graph contains cycles then returns none. -/
def topSort3 (g : Graph α β) : Option (Array Nat) :=
  let res : Option State := g.depthFirstTraversal4 g.getAllVertexIDs initialState (visit g) leave
  match res with
    | some state => some state.res.reverse
    | none => none
  where
...
/-- Retruns a topological ordering of any DAG (directed acyclic graph). If the graph contains cycles then returns wrong results. -/
def topSort4 (g : Graph α β) : Option (Array Nat) :=
  let res := g.depthFirstTraversal4 g.getAllVertexIDs initialState (visit g) leave
  res.reverse
  where
    initialState := #[]
